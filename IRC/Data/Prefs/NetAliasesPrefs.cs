...
public class NetAliasesPrefs : GlobalAliasesPrefs
{
	#region Constructors & Deconstructors
		public NetAliasesPrefs(in NetPrefsBase mgrParent, in GlobalAliasesPrefs inheritedSettings, in string? strName =
				null, in string? strLocalizedName = null, in string? strLocalizedDesc = null) :
			base(mgrParent, strName ?? "Alias overrides for this network", strLocalizedName ?? PrefsRsrcs.strNetAliasTitle,
				strLocalizedDesc ?? PrefsRsrcs.strNetAliasDesc)
		{
			this.mgrParent = mgrParent;
			this.inheritedSettings = inheritedSettings;

			System.Collections.Generic.IEnumerable<NetInheritedItemEnabledStatus<GlobalAliasesOneAlias, IReadOnlyOneAlias>>
			...
			mapAllInheritanceOverridesByName = new(
				...
				(inherited, evth)
					=> inherited.evtKeyOfInheritedItemChanged += mapOverrideHandlers[evth] = (in
							NetInheritedItemEnabledStatus<GlobalAliasesOneAlias, IReadOnlyOneAlias> _, in string strOldName,
							in string _)
			...
			addedAliases = new(
				...
				(aliasEntry,
						evth)
					=> aliasEntry.evtNameChanged += mapAddedAliasesHandlers[evth] = (in GlobalAliasesOneAlias _, in
		...
		public NetAliasesPrefs(in NetPrefsBase mgrParent, DTO.NetAliasesDTO dto, in GlobalAliasesPrefs inheritedSettings, in
				...
				strLocalizedDesc ?? PrefsRsrcs.strNetAliasDesc)
		{
			...
			mapAllInheritanceOverridesByName = new(
				...
				(inherited, evth)
					=> inherited.evtKeyOfInheritedItemChanged += mapOverrideHandlers[evth] = (in
							NetInheritedItemEnabledStatus<GlobalAliasesOneAlias, IReadOnlyOneAlias> _, in string strOldName, in
							string _)
						=> evth(strOldName, inherited),
				(inherited,
						evth)
	...
	#region Members
		...
		private readonly Platform.DataAndExt.Prefs.MappedObjListItem<string,
			NetInheritedItemEnabledStatus<GlobalAliasesOneAlias, IReadOnlyOneAlias>> mapAllInheritanceOverridesByName;

		private readonly System.Collections.Generic.Dictionary<System.Action<string,
			NetInheritedItemEnabledStatus<GlobalAliasesOneAlias, IReadOnlyOneAlias>>,
			NetInheritedItemEnabledStatus<GlobalAliasesOneAlias,IReadOnlyOneAlias>.DFieldChanged<string>> mapOverrideHandlers
			= [];

		private readonly Platform.DataAndExt.Prefs.MappedSortedListItem<string, GlobalAliasesOneAlias> addedAliases;

		private readonly System.Collections.Generic.Dictionary<System.Action<string, GlobalAliasesOneAlias>,
			GlobalAliasesOneAlias.DFieldChanged<string>> mapAddedAliasesHandlers = [];
	#endregion

	#region Properties
		public Platform.DataAndExt.Prefs.MappedObjListItem<string, NetInheritedItemEnabledStatus<GlobalAliasesOneAlias,
