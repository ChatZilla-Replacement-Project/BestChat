...
public class ChanStalkWordsPrefs : Platform.DataAndExt.Prefs.AbstractChildMgr, IStalkWordsPrefs
{
	#region Constructors & Deconstructors
		public ChanStalkWordsPrefs(ChanPrefs mgrParent, NetStalkWordsPrefs inheritedSettings) :
			base(mgrParent, "Stalk words for this channel", PrefsRsrcs.strNetChanStalkWordsInheritedTitle, PrefsRsrcs
				.strNetChanStalkWordsInheritedDesc)
		{
			this.inheritedSettings = inheritedSettings;

			System.Collections.Generic.List<ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord,
				IReadOnlyOneStalkWord>> listDefEnabledAliases = inheritedSettings.AllInheritanceOverridesByCtnts.Values
				.Select(iswCur
					=> new ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord, IReadOnlyOneStalkWord>(iswCur
						.inheritedItem, iswCur.Status,ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord,
						IReadOnlyOneStalkWord>.InheritedFromTypes.global, PrefsRsrcs.strStalkWordsText, inheritedSettings.mgrParent
						.OwnerNet)
				).ToList();
			listDefEnabledAliases.AddRange(inheritedSettings.AddedStalkWords.Values.Select(iswCur
				=> new ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord, IReadOnlyOneStalkWord>(iswCur, true,
					ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord, IReadOnlyOneStalkWord>.InheritedFromTypes
			...
			mapAllInheritanceOverridesByName = new(
				...
				(inherited,
						evth)
					=> inherited.inheritedItem.evtCtntsChanged += mapOverrideHandlers[evth] = (in GlobalStalkWordsOneStalkWord _,
							in string strOldVal, in string _)
				...
				.strNetChanStalkWordsInheritedDesc)
		{
			...
			System.Collections.Generic.List<ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord,
				IReadOnlyOneStalkWord>> listDefEnabledStalkWords = inheritedSettings.AllInheritanceOverridesByCtnts.Values
				.Select(iswCur
					=> new ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord, IReadOnlyOneStalkWord>(iswCur
						.inheritedItem, iswCur.Status,ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord,
						IReadOnlyOneStalkWord>.InheritedFromTypes.global, PrefsRsrcs.strStalkWordsText, inheritedSettings.mgrParent
			...
			foreach(ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord, IReadOnlyOneStalkWord> ialiasCur in
					listEnabledInheritedStalkWords)
				ialiasCur.evtDirtyChanged += (in NetInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord,
						IReadOnlyOneStalkWord> _, in bool bNowDirty)
					=>
						{
							if(bNowDirty)
								MakeDirty();
						};
			listEnabledInheritedStalkWords.AddRange(inheritedSettings.AddedStalkWords.Values.Select(iswCur
					=> new ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord, IReadOnlyOneStalkWord>(iswCur,
	...
	#region Properties
		public Platform.DataAndExt.Prefs.MappedListItem<string, ChanInheritedItemEnabledStatus<GlobalStalkWordsOneStalkWord,
			IReadOnlyOneStalkWord>> AllInheritanceOverridesByName
				=> mapAllInheritanceOverridesByName;
