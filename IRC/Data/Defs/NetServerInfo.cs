...
public class NetServerInfo : Platform.DataAndExt.Obj<NetServerInfo>
{
	...
	#region Methods
		...
		protected void FirePortsChanged(in System.Collections.Generic.IReadOnlyCollection<ushort>? eNewPorts = null, in
			System.Collections.Generic.IReadOnlyCollection<ushort>? eRemovedPorts = null)
		{
			FirePropChanged(nameof(Ports));

			evtPortsChanged?.Invoke(this, ussetPorts, eNewPorts, eRemovedPorts);
		}

		protected void FireSslPortsChanged(in System.Collections.Generic.IReadOnlyCollection<ushort>? eNewPorts = null, in
			System.Collections.Generic.IReadOnlyCollection<ushort>? eRemovedPorts = null)
		{
			FirePropChanged(nameof(SslPorts));

			evtSslPortsChanged?.Invoke(this, SslPorts, eNewPorts, eRemovedPorts);
		}
		...
		protected void RemovePort(in ushort usPortToRemove)
		{
			if(ussetPorts.Remove(usPortToRemove))
			{
				...
				FirePortsChanged(eRemovedPorts: [usPortToRemove,]);
			}
		}
		...
		protected void ClearPorts()
		{
			...
			{
				System.Collections.Generic.IReadOnlySet<ushort> ussetOldPorts = ussetPorts;

				ussetPorts.Clear();

				MakeDirty();

				FirePortsChanged(eRemovedPorts: ussetOldPorts);
			}
		}
		...
		protected void RemoveSslPort(in ushort usSslPortToRemove)
		{
			if(ussetSslPorts.Remove(usSslPortToRemove))
			{
				...
				FireSslPortsChanged(eRemovedPorts: [usSslPortToRemove,]);
			}
		}
		...
		protected void ClearSslPorts()
		{
			...
			{
				System.Collections.Generic.IReadOnlySet<ushort> ussetOldSslPorts = ussetSslPorts;

				ussetSslPorts.Clear();

				MakeDirty();

				FireSslPortsChanged(eRemovedPorts: ussetOldSslPorts);
			}
		}
